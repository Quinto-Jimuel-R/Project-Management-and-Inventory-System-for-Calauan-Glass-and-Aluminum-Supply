<?php
    ...
    $customerResult = mysqli_query($conn, "SELECT * FROM user WHERE user_id = $customer_id");
                ...
                <div class="card border-0 bg-primary rounded-0">
                    <div class="fixed-header p-3">
                        <div class="d-flex align-items-center gap-3">
                            <a href="user_message.php"><i class="fa-solid fa-arrow-left"></i></a>
                            <img src="images/<?php echo $customerData['image'] ? $customerData['image'] : $defaultImage; ?>" alt="Current Image" style="border-radius: 50%; width: 45px; height: 45px;">
                            <div class="d-flex align-items-center" style="font-weight: 700;"><?php echo htmlspecialchars($customerData['name'], ENT_QUOTES, 'UTF-8'); ?></div>
                        </div>
                    </div>
                    ...
                    <?php
                        ...
                        $messageResult = mysqli_query($conn, "
                            ...
                            AND ((`from` = $employeeID AND `to` = $customer_id) 
                                OR (`from` = $customer_id AND `to` = $employeeID))
                        ...
                        <div class="messages-container">
                            <?php foreach ($messages as $index => $message): ?>
                                <?php
                                    ...
                                    $messageClass = ($message['from'] == $employeeID) ? 'message-from' : 'message-to';
                    ...
                    <form action="add.php?action=insertMessage" class="input-container" id="messageForm" name="messageForm" method="POST">
                        ...
                        <input type="hidden" name="customerID" value="<?php echo $employeeID ?>">
                        <input type="hidden" name="employeeID" value="<?php echo $customer_id ?>">
