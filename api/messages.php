...
$query = "SELECT m1.*, u.name AS customer_name, u.image AS customer_image, p.project_name, p.project_id, u.user_id AS customer_id, e.user_id AS employee_id
    FROM message m1
    JOIN user u ON (m1.from = u.user_id OR m1.to = u.user_id)
    JOIN user e ON (m1.from = e.user_id OR m1.to = e.user_id) -- Add this join to get employee details
    JOIN project p ON m1.project_id = p.project_id
    WHERE (m1.from = '$customerID' AND m1.to = u.user_id) 
       OR (m1.to = '$customerID' AND m1.from = u.user_id)
...
while ($message = mysqli_fetch_assoc($messagesResult)) 
{
    ...
    $employeeID = $message['employee_id']; // Fetch employee_id
    ...
    if (!isset($messagesByProject[$projectID])) 
    {
        ...
        $messagesByProject[$projectID] = [
            ...
            'is_employee_sender' => $message['from'] == $customerID,
                    ...
                    <div class="card border-0 mb-4">
                        ...
                        <?php if (!empty($messagesByProject)): ?>
                                <?php foreach ($messagesByProject as $projectID => $projectData): ?>
                                    <a href="message.php?project_id=<?php echo urlencode($projectID); ?>&customer_id=<?php echo urlencode($projectData['employee_id']);?>&employee_id=<?php echo urlencode($projectData['message']['customer_id']);?>" style="text-decoration: none;">
